...
d3.json("./data/cyclist-data.json").then(function (json) {
  var xScale = d3.scaleTime().domain([d3.min(json, function (d) {
    return new Date(d.Year, 0);
  }), d3.max(json, function (d) {
    return new Date(d.Year, 0);
  })]).range([0 + padding + yAxisPadding, w - padding]);
  var yScale = d3.scaleLinear().domain([d3.min(json, function (d) {
    return d.Seconds;
  }), d3.max(json, function (d) {
    return d.Seconds;
  ...
  chart.selectAll("circle").data(json).enter().append("circle").attr("cx", function (d) {
